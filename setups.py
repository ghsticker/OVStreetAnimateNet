...
background = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/background.png'))).float(), dim=2) < 100).float()

island11 = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/1island1.png'))).float(), dim=2) < 100).float()
island12 = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/1island2.png'))).float(), dim=2) < 100).float()
island13 = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/1island3.png'))).float(), dim=2) < 100).float()
island21 = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/2island1.png'))).float(), dim=2) < 100).float()
island22 = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/2island2.png'))).float(), dim=2) < 100).float()
island23 = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/2island3.png'))).float(), dim=2) < 100).float()
island31 = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/3island1.png'))).float(), dim=2) < 100).float()
island32 = (torch.mean(torch.tensor(np.asarray(Image.open('imgs/3island2.png'))).float(), dim=2) < 100).float()

images = {"island11":island11,"island12":island12, "island13":island13,"island21":island21, "island22":island22,"island23":island23,"island31":island31,"island32":island32}

backgrounds = {"empty":background}
...
class Dataset:
	def __init__(self,w,h,batch_size=100,dataset_size=1000,average_sequence_length=5000,interactive=False,max_speed=3,brown_damping=0.9995,brown_velocity=0.005,init_velocity=0,init_rho=None,n_cond=False,dt=1,types=["box","DFG_benchmark"],images=["island11","island12"],background_images=["empty"]):
		"""
		create dataset
		:w: width of domains
		:h: height of domains
	...
	def reset_env(self,index):
		...
		if type == "box":# block at random position
			...
			# object_h = 20  # object height / 2
			# object_w = 15  # object width / 2
			...
			self.v_cond[index,1,:,0:5]=flow_v
			self.v_cond[index,1,:,(self.w-5):self.w]=flow_v
